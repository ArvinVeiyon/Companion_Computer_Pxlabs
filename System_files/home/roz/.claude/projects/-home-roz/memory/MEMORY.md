...
- ⚠️ **2026-07-26 CAMERA REMOUNT (bracket printing) — new TF already written to `depth_to_scan.launch.py`: cam_x 0 (rotation centre), cam_z 0.305, range_max 5.0. pitch/roll are TARGETS — re-derive from `/camera/accel/sample` + `systemctl restart rover-scan` after fitting. Rover dims re-measured (supersede 07-21). BLOCKS L5 until as-built verified → see [[project_l4_gemini_nav2_prereqs]]**
...
Cameras (roles set 2026-07-19): LG Smart Cam=FPV alias FPV, id usbcam-30c9009d-01.00.00-i00 (video8 today, 1280x720 MJPG user-applied from QGC; stable ids survive boot shuffles since v2.1) | Orbbec Gemini 336L=autonomy-only, color alias NAV-COLOR role_lock, id usbcam-2bc50807-CPC7B53000AB-i04 (**has NO /dev/videoN at all** — OrbbecSDK/libusb, not uvcvideo; the old "video6/video0=depth Z16/video2/4=IR" note was WRONG, verified 2026-07-27. **UNPLUGGED 2026-07-27 eve** for video debug, rover-camera+rover-scan stopped → blocks L5, restart to restore; up to 1280x800 MJPG; USB3 on BOX-B; ROS2 wrapper for phase3/4, never ffmpeg). **/dev/video renumbering is caused by the Pi5's own `rpivid`+`pispbe-*` axi nodes (video19-37), NOT by the Orbbec** — LG cam was video8 at boot, video0, then video1 in one evening | old front/bottom cams removed | NEVER key cameras by /dev/v4l/by-id (index order not boot-stable)
