...
- `project_rover_autonav.md` — **ACTIVE, RESUME HERE**: rover autonomous nav, layered flow L0-L7 (spec = ros2_ws/docs/rover_autonav_requirements.md). L0 done (px4_msgs+lib pinned release/1.17, ws rebuilt 26 pkgs). L1 built: **next action = `ros2 run autonav_mode autonav_mode` → verify registration → user checks "AutoNav" in QGC**. Then L2 wheels-up /cmd_vel test. Uncommitted: src/autonav_mode, src/rover_odometry, docs edits. See file for full pickup steps
