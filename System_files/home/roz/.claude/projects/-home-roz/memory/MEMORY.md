...
- `project_wfb_undervoltage_dead_nic.md` — **2026-07-25 WFB dropouts: dead NIC wlx782288d993c0 (99.8% TX drop) REMOVED from WFB_NICS 11:08 → 0 drops, flapping gone, now single-adapter (no TX diversity). ROOT CAUSE STILL OPEN = 5V INPUT SUPPLY SAG, EXT5V measured 4.65-4.84V (healthy=5.0-5.1, trips ~4.8), tracks VDD_CORE_A. NOT a USB-budget problem — WFB NICs are externally powered behind a SELF-POWERED hub, zero over-current in dmesg. Fix = PSU/cable/BEC. **DO NOT set usb_max_current_enable=1** (makes sag worse)**
