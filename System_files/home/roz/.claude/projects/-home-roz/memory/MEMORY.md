...
## [MEMORY_FILES]
**feedback (RULES):** `check_docs_before_measuring` 🔴 **grep the WHOLE memory dir + `docs/` for a dimension BEFORE deriving it; an index line must name a file's LOAD-BEARING content; the operator's ID of their own hardware beats my inference** · `eliminate_hypothesis_whole_family` 🔴 **dump EVERY param that could implement X in ONE command before saying "not the cause"; name the CONSUMER, not a bare yes/no; and a real gap ≠ a significant one — measure its effect** · `crash_recovery_checkpoint` (**checkpoint PER FINDING**) · `use_dds_not_mavlink` (**FC over DDS, never MAVLink probing**) · `camera_qgc_only` (**FPV cams set ONLY from QGC**) · `dkms_arch` · `wlan0_persistent_name` (udev rule GONE 07-30)
**reference:** `px4_vio_collision` (🔴 **PX4 collision-prevention is MC-ONLY, dead on a rover; no PX4 path planner; `EKF2_EV_CTRL=4`=velocity-only ⇒ cannot fix Q1**) · `wfb_ng` · `wfb_rlyctl` · `wfb_cfg_apply` · `uart_map` · `services` · `known_fixes_archive` · `gcs_companion_interface` · `todos.md` · `ros2_nodes`/`ros2_topics` · `rover_odometry`
**project — ACTIVE:** `indoor_mapping_slam` (🔴 **`house_map.pgm` IS FALSE — 38 unchecked closures folded it. Wheel-odom replay = 60 closures vs 4-5. Remaining gap: WALLS NOT MARKED OCCUPIED**) · `rover_autonav` (**🔴 YAW RUNAWAY ~21×, ⛔ no armed yaw tests; ALL pre-08-01 speeds read 12.2× LOW**) · `perception_3d_costmap` (**`/scan_3d` live, rate fixed, top-plate settled; reflex NOT switched**) · `autonomy_plan_reframe` (**Q1 localization = THE WALL**) · `rc_control_camera_retry_storm` (**FIXED `9893d6b` — WAS the "runaway" blamed twice on a stray process**)
...
## [CURRENT STATE — 2026-08-08 01:15. Mapping session; NOTHING on the rover changed, never armed]
**All work was OFFLINE** on bags/dbs. Live stack untouched: core + autonav svcs active, DISARMED. `vision_streaming` was ACTIVE at 23:42 (ffmpeg ~27%) and **`inactive` by 01:40 — I never touched it; unexplained, check it before assuming a fault.** **Map replays run on `ROS_DOMAIN_ID=42` — mandatory, domain 0 has the live `/tf` + `/camera/*`.** New files in `~/`: `rtabmap_replay_wheel{,_grid}.db`, `sweep_roi_*.db`, `map_export/`; **`rtabmap_run3_{fixedA,B1,B2,C,E}` are superseded — safe to delete.** ⚠️ `MEMORY.md` is **19.5 KB, OVER the 17 KB ceiling** — needs a trim pass.

## [PREVIOUS STATE — 2026-08-01 22:30, live-verified. Nothing broken]
...
🔑 **MANUAL MODE BYPASSES THE YAW RATE LOOP** (fw-verified: `manual()` publishes STEERING, only `acro()` a RATE setpoint) ⇒ **MAPPING IS NOT YAW-GATED — drive it in Manual NOW.** ⚠️ **the collision reflex does NOT apply in Manual** (it lives in `autonav_mode`'s custom-mode executor) — **the operator is the only safety layer.** #20 still gates every AUTONOMOUS mode.
📦 **OFFLINE ARCHITECTURE: record on the Pi → map on a LAPTOP → localization-only on the Pi** (localizing is far cheaper than mapping). `ros2 bag record` = **42.6% of a core, does NOT degrade `/scan`** (14.2 vs 13.4 Hz). 🔴 **DISK is the constraint: 29.3 MB/s vs a 27.4 MB/s SD ceiling ⇒ writes AT the limit; 27 GB ≈ 15 MIN. USB SSD effectively required.** ⚠️ live-streaming RGB-D is NOT viable — `image_transport` has **only `raw_pub`** (no compressedDepth) ⇒ ~100 Mbit/s vs a 13 Mbit/s link.
📊 **RTAB-Map installed, MEASURED: does NOT fit realtime.** `rgbd_odometry` alone = **79.6% of a core**, load 2.25→9.44, **`/scan` HALVED** 13.4→7.0 Hz, its own delay ~1 s at ~3 Hz — **without** the map/loop-closure node or the `depth_registration` RGB-D needs. ⏭ cheapest lever: **colour 1280×720 → 640×360** (~4×; also shrinks the bag). Launch-time ⇒ camera restart ⇒ **run the half-dead check**.
🔴 **Mapping is NOT hardware-blocked:** `slam_toolbox` needs 360°, but **RTAB-Map RGB-D is designed for a narrow forward FOV** and closes loops by *recognising places*. **Swap the ALGORITHM, not the sensor.** ⚠️ **92° is NOT a weakness of the 336L — it's the whole RGB-D class** (D455 ≈87°); the 336L is wider AND its 0.308 m near limit beats the D455's ~0.5 m, which is what keeps the blind circle INSIDE the bumper.
⚠️ **PERMANENT CONSTRAINTS (camera-only):** no rear/side coverage ⇒ **"never reverse into unseen space" is permanent**; **never clear a spin from any `/scan`**; ~3 m range costs map quality in big rooms. A 2nd STL-19 removes the choice.
⏭ **MAPPING STATUS SUPERSEDED 08-08 → `project_indoor_mapping_slam.md`.** Plan Steps 1-3 ✅ and **Step 5 ✅ PASSED** (camera reconfig 640×360 + `depth_registration` was ALREADY DONE before 08-02 — the plan's "next" list is stale). #20 yaw still gates autonomous driving. **O1 removed from the rover ladder**; lidar-owns-`/scan` remap PARKED.
> `docs/roadmap.md` (07-23) vs **`docs/autonomy_plan.md` (08-01, NEWER: L0 ✅ · L1 🔧 · L2-L5 NOT STARTED)** — **different L-numbers, don't mix them.** 🔴 **Q1 "where am I?" IS THE WALL.** PX4 RTL has NO obstacle avoidance here. Pending hw: DroneCAN GPS.
