...
- `reference_wfb_rlyctl.md` · `reference_wfb_cfg_apply.md` · `reference_uart_map.md` · `reference_services.md` · `reference_known_fixes_archive.md` · `reference_gcs_companion_interface.md` · `todos.md` · `ros2_nodes.md` / `ros2_topics.md` (nodes / DDS topics) · `rover_odometry.md`
...
- `project_l2_floortest_wheel0_reversed.md` (L2 PASS + reflex collision-stop; wheel-0 "reversal" was a FALSE ALARM) · `project_l4_gemini_nav2_prereqs.md` (L4 DONE; Nav2 1.3.12 + slam_toolbox 2.8.5) · `project_vision_multicam_upgrade.md` (A+B+C done, **Phase D remains**)
- `project_ffmpeg_hung_alive_gap.md` — **READ ITS 08-01 SECTION FIRST.** CPU-starvation latch, the vision-node defect list, the software-recovery-doesn't-work proof, "Pi 5 has no HW H.264 encoder"
- `project_wfb_undervoltage_dead_nic.md` — LIKELY FIXED 07-25 (XL4015 @5.25V). **DON'T raise the pot; DON'T set usb_max_current_enable=1.** `ext5v-report` reads the rail UPSTREAM — blind to drops at a connector
- `project_external_wifi_uplink.md` — RTL8821CU `wlx90de80d824d6` = PRIMARY uplink @ 192.168.1.240; holds the `disable-wifi-pi5` story
- `project_gcs_link_degraded.md` — **07-31: downlink CLOSED (~100%; the "15%" was a bad method). Uplink 13.6% lossy → drone antenna.**
- `project_relay_ntp_setup.md` — relay clock, OPEN (recurred 07-11) · `project_relay2_relaystn.md` — RELAY-STN RPi4, OPEN: WFB card browns out the Pi4 USB budget, fix = powered hub
...
**(B) 🔴 CPU-STARVATION LATCH (08-01) — looks identical, is NOT a camera fault. CHECK THIS FIRST.** ffmpeg loses a CPU race to the rover stack and **never recovers**: ~3 s bursts every ~14 s, **7-28 pkt/s vs 208 healthy**, silent >1 h. **Restarting the service does NOT clear it. FIX: briefly `systemctl stop rover-camera rover-scan rover-odometry`** → 7 → 208 pkt/s, stays healthy after they restart. Now auto-caught by the throughput-floor watchdog (todo #23).
**⚠️ MEASUREMENT TRAPS (both in the topic file):** per-second `video tx incoming` reads 0 inside the gaps — **use a CUMULATIVE delta over ≥30 s**; and a **manually launched** ffmpeg never moves WFB's counter though the service's does — **never A/B ffmpeg flags via that counter.**
**BEST DIAGNOSTIC:** stop the service, run `ffmpeg -loglevel verbose` by hand, read `N packets read; N frames decoded` + `*** N dup!`. **x264 dup-pads ⇒ live RTP ≠ live camera**; confirm at the GS. Healthy = 152 packets/10 s, 0 decode errors. `unable to decode APP fields` + ~5 `No JPEG data found` in the first second are BENIGN. **⚠️ Stop the service before touching v4l2 controls** (`auto_exposure` mid-stream stalls it >12 s).

## [KNOWN_FIXES] → reference_known_fixes_archive.md · latest: camera identity 07-19 (usbcam sysfs ids, vision_config_manager v2.2.3) · open regression: relay NTP fix didn't hold
...
**⚡ 07-31 DEFINITIVE (20 min, both ends, under full video load):** downlink **99.86-99.99%** delivered; uplink GS→drone loses **13.57% mavlink / 5.46% tunnel**, continuous not bursty. **ROOT CAUSE = drone NIC-A ant0 −48.5 vs ant1 −28.3 dBm (20 dB deaf), 224 samples; GS antennas identical. NIC-B only 3 dB, NOT 9.** ⇒ **ONLY WFB JOB LEFT: reseat that u.FL/pigtail/antenna, re-measure.** ✅ DELETED as causes: ring-buffer/EAGAIN (#3), GS TX power (#4, already 30 dBm max), peer `10.5.6.50` (correct — QGC laptop on the relay hotspot; ping blocked by Windows firewall).
⚠️ **MEASUREMENT METHOD (cost a week):** compare payload `tx.incoming`→`rx.out` across 8102/8103. **Never `rx.all`** (drone double-counts 4 antennas: 13.6% looks like 2.4%). **Never infer radio health from MAVLink rates at two `tcp:5760` endpoints** — that path includes mavlink-router + PX4 config, and produced the bogus "15% downlink".
...
3. **✅ Memory mirrored AND PUSHED 08-01** — `~/codex-work/memory/` @ `205da9b`, `549d4d4..205da9b`, 0 ahead. **⚠️ The push used the plaintext PAT still embedded in the remote URL (user chose to push anyway) ⇒ todo #14 (rotate + move to SSH) is now MORE urgent, not less.** Copy method: `cp -p`, never `rsync --delete` (mirror is a UNION — 3 files belong to the codex-work scope). **UNDECIDED: user said "del mirror one" then deferred — ask before deleting anything.**

## [TODOS]
→ See memory/todos.md (full detail + commands)
**🔴🔴 WFB-NG IS THE ACTIVE PRIORITY (user, 07-31). NEXT ACTION IS A HARDWARE JOB: reseat the drone's NIC-A ant0 u.FL + pigtail + antenna (~20 dB deaf), then re-measure on 8102** (= todo #22). W0-W6 otherwise closed. After it: re-measure uplink loss, then test possible co-located desense (relay TX 31 dBm ch149 vs WFB RX ch161). Relay journal still unread — **`vind-admin` sudo needs a password.**
**Rover: next = #20 yaw gains, then L5.** #21 gyro-yaw odom open. **🔴 NEW L5 BLOCKER 08-01: `/odom` dies at rest (ESC doze).** **[OUTDOOR = PRIMARY TARGET] O1-O5** after L5/L6: STL-19 · DroneCAN GPS · lidar SLAM · GPS-waypoint Nav2 · outdoor safety
...
5+7+8. Antenna tracker HW (relay :14551) · /scan → obstacle_distance/Nav2 · #17 delete camera_sw_node_obsolute.py · Multicam Phase D (rc_control yamls + optflow → usbcam ids)
9+10. Vision open: (a) backoff reset on the STALL path; (b) pin `6-2/power/control` to `on` (hygiene); (c) vision_config_manager v2.3.0 `--bitrate` (designed, not written). **FPV soak ✅ PASSED 07-31** (41.8 min clean) — only the reboot check remains.
22. **🔴🔴 THE #1 WFB ACTION — numbers in the WFB_NG block above.** Reseat NIC-A ant0.
23. ✅ **Throughput-floor watchdog APPLIED+VERIFIED+PUSHED 08-01** (`a5fb348`): FLOOR 5 fps / WINDOW 20 s / GRACE 30 s; no backoff reset on that path. **⛔ USER RULE: NEVER modify the ffmpeg command line** (`vision_streaming_node.py` ~176-207) — `-g 30`, `-tune zerolatency`, `-pkt_size 1400`, `fps`→`-framerate` are **VETOED.** Still open (non-ffmpeg): `frame=0` grace bug; `rclpy.shutdown()` traceback on every QGC camera change; cap cold-start retries
...
Cameras (**both FPV-capable, both proven on port 6-2; swap from QGC only**) — **See3CAM_CU135** `usbcam-2560c1d1-241D8306-i00` (100mA): real **60 fps** 720p MJPG over USB 2.0, no USB3 needed (old "~16 fps" note was WRONG — auto-exposure in a dark room); ~16 fps indoors at night. **LG Smart Cam** `usbcam-30c9009d-01.00.00-i00` (500mA): **30 fps**, healthy 08-01, cheaper on CPU (78% of a core vs 95%). | **Orbbec Gemini 336L** = autonomy-only, alias NAV-COLOR role_lock, `usbcam-2bc50807-CPC7B53000AB-i04` (USB3 on BOX-B, ROS2 wrapper only, never ffmpeg)
