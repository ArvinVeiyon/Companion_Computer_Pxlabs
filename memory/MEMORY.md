...
⏭⏭ **FOCUS (operator): AUTONAV.** The plan is `todos.md` §REQUIREMENTS REALIGNMENT + START HERE. 🔑 **GATES IN DEPENDENCY ORDER G0→G2→G3.** **G0** ✅ 09-11. 🔴🔴 **G2 IS MEASURED OUT 09-12 — IT IS NOW A DECISION, NOT A TEST: duty mode (USB reflash, throttle means speed) vs rebuild the loop around torque. NOTHING ABOVE M1 IS SAFE UNTIL IT IS MADE.** ⬜ also open in G2: brake→reflex wiring · zero-dropout on the floor · T1 RE-EXAMINED not re-ticked. **G3** = ⏭ **OPERATOR VENUE CALL** (still gates T2).
