...
- `project_rover_autonav.md` — **ACTIVE. 🔴 07-29: YAW RATE RUNAWAY — commanded 0.3 rad/s, ACTUAL ~6.3 rad/s (~21x, and 4x past `RO_YAW_RATE_LIM` 1.57); gyro + /odom + user's own eyes agree. NOT a gain trim — the loop isn't controlling. ⛔ NO armed yaw tests until fixed.** Also 🔴 **`erpm_to_ms: 0.000380` is ≥2.3x too small** ⇒ /odom under-reports speed, contaminating EKF2 EV velocity and (later) Nav2+SLAM — **fix by driving a tape-measured 2 m and comparing /odom, BEFORE L5.** Arm in **Manual** via RC → software DO_SET_MODE→AutoNav. **Hazards:** wheels-up + ekf-bridge + closed loop = limit cycle, only disarm stops it; never `pkill -f`/`pgrep -f` (self-matches). ESC sleep at rest ⇒ check `esc_online_flags==15` before trusting `/odom`. **NEW 07-28 — two traps:** (1) ekf-bridge is velocity-only ⇒ **EKF `eph` grows unbounded** (hit 682 m vs `COM_POS_FS_EPH` 5 m) ⇒ **armed** entry to AutoNav/Hold silently refused while a **disarmed** dry-run still passes; fix = reboot FC, and it WILL recur. (2) **rover hit a wall with the collision-stop working** — yaw was ungated (asymmetric skid-steer spin translates) and thresholds were camera- not bumper-referenced; both fixed in ros2_ws `7261fc7` (**unpushed**)
