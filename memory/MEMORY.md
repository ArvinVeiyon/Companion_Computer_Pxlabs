...
**reference:** `px4_vio_collision` (🔴 **PX4 collision-prevention is MC-ONLY, dead on a rover; no PX4 path planner. `EKF2_EV_CTRL` is a BITMASK — ours=4=velocity-only; `=9` (pos+yaw) is the fix**) · `wfb_ng` · `wfb_rlyctl` · `wfb_cfg_apply` · `uart_map` · `services` · `known_fixes_archive` · `gcs_companion_interface` · `todos.md` · `ros2_nodes`/`ros2_topics` · `rover_odometry`
...
⛔ **Do NOT modify the ffmpeg command line** (vetoed) · ⛔ **never hardcode frame rate; the `fps` conf key is INERT** · **never record resolution/fps/bitrate as fact — read live.**
...
core: mavlink.router | microxrce-agent | rc_control_node | vision_streaming | block-traffic | wifibroadcast@drone | system_files_sync.timer | ollama · autonav: rover-{camera,scan,scan-3d,odometry,autonav-mode}
...
🔴 **A CAMERA RESTART CAN COME UP HALF-DEAD** — "active", params answering, **depth & colour never started, nothing logged**; a 2nd restart fixed it. `systemctl is-active` does NOT catch it ⇒ **verify EVERY restart: `python3 ~/ros2_ws/tools/camera_restart_check.py`.**
...
**CAMERA STAYS CENTRED** — keeps the 0.300 m min-range circle 45 mm BEHIND the bumper = no blind strip; moving it breaks `cam_x=0` + `front_overhang`/TF. ⚠️ **MY recommendation, operator-accepted; REOPEN if its DERIVED basis falls.**
...
## [CURRENT STATE — 2026-08-08 21:30, session end. NEVER ARMED]
✅ **#26 + #28 done, committed AND PUSHED.** Camera 30/30, restart verified, `/scan` 26.3 Hz. **`vision_streaming` STOPPED by the operator 21:05; rtabmap KILLED at session end** (it had relocalized; parked, it could progress no further). Rover parked ROOM CENTRE facing the drawer tower.
⏭⏭ **RESUME HERE — 26b is ONE HAND-PUSH AWAY.** Relaunch localization (exact command in indoor_mapping_slam §12), **push the rover a metre by hand**, watch the 2nd fix commit. **Grep `Rtabmap.cpp:3772` / "Localization was good" — NOT "accepted loop closure".**
⚠️ **sudo NEEDS the password** — `printf '1987\n' | sudo -S …`. ⚠️ **`fps` INERT in QGC**. **Replays need `ROS_DOMAIN_ID=42`; LIVE stack is domain 0.** 🔴 **#14 MORE URGENT — the 08-08 push used the plaintext PAT again. ROTATE IT.**
🖥 **Pi5 is NOT the bottleneck (measured 08-08): 5-41% idle, ~0% iowait, RAM 1.8/7.9 GB; everything heavy is ALREADY multi-threaded** (camera 41 threads, rtabmap 27, ffmpeg 19) ⇒ **more threads cannot help.** ⛔ **NO GPU ENCODE EXISTS — `h264_v4l2m2m`="no valid device", `h264_vaapi`=no driver, card0=`v3d` (3D only). Being headless changes NOTHING. Only real offload = a camera with ONBOARD H.264.** 🔑 CPU levers: `align_mode:=HW` (71%/core) · wheel_odometry C++ port (20%/core; **65% is rclpy executor overhead, ~4% our logic — NOT logging**).
...
Cameras (**both FPV-capable, port 6-2; swap from QGC only**): **See3CAM_CU135** `usbcam-2560c1d1-241D8306-i00` 60 fps 720p MJPG · **LG Smart Cam** `usbcam-30c9009d-01.00.00-i00` 30 fps, cheaper CPU, **the current FPV cam** · **Orbbec Gemini 336L** autonomy-only `usbcam-2bc50807-CPC7B53000AB-i04` (USB3 BOX-B, wrapper only, **never ffmpeg**), min depth 0.308 m
...
🔑 **PX4 MANUAL BYPASSES THE YAW RATE LOOP** (fw-verified: `manual()` publishes STEERING, only `acro()` a RATE sp) ⇒ mapping drives are NOT yaw-gated, **but the reflex doesn't run there either — the operator is the only safety layer.** AutoNav publishes speed+RATE ⇒ the yaw loop gates EVERY AutoNav task.
...
> `docs/roadmap.md` (07-23) vs **`autonomy_plan.md` (08-01, NEWER)** — **different L-numbers, don't mix.**
