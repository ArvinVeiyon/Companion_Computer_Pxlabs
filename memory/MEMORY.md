...
⏭ **ORDER: USB SSD → 640×360 colour + `depth_registration` (one restart) → manual mapping run → offline RTAB-Map on the laptop → AMCL/Nav2 on the Pi.** #20 yaw still gates autonomous driving. **O1 removed from the rover ladder**; lidar-owns-`/scan` remap PARKED.
