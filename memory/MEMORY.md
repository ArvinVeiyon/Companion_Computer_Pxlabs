...
🗂️ **`$HOME` layout, the moved map, the 85%-full disk → `this_machine`.** ⛔ `ros2_ws` + `codex-work` did NOT move.
...
✅✅ **BRAKE vs COAST — MEASURED 09-12, BOTH TO A FULL STOP: brake 1.44 m/s² (0.22 m from 0.8 m/s) vs coast 0.46 m/s² (0.70 m) ⇒ 3.1×.** Supersedes "3.4× provisional". 🔴 **THE BRAKE TAKES ~0.3 s TO BITE.** 🔑 regen capped **−4.99 A ⇒ `l_in_current_min` −5 A binds**; authority FADES as it slows. ⛔⛔ **ZERO THROTTLE = FREE COAST — 1.78 m from 1.40 m/s** ⇒ the standoff is SPEED-BOUND.
🔴 **THE REFLEX STILL CANNOT BRAKE.** Slot 5 = RC passthrough. ✅ **09-12 `UAVCAN_EC_FUNC6`=301 set + persisted** ⏭ **INERT until the 4 VESCs read RawCommand idx 5.** ⛔ −1.0=off, 0.0≈50%, value LATCHES. 🔑 **ESC config: `uavcan_raw_mode` 3 = RPM would fix G2 at the ESC and give a real speed cap — UART or USB, one session per ESC, NOT over CAN, and ⛔ BLE is out (module sits on the DRV SPI).** → `project_vesc_can_flashing` 09-12
...
⏭⏭ **FOCUS (operator): AUTONAV.** Plan = `todos.md` §REQUIREMENTS REALIGNMENT + START HERE. 🔑 **GATES G0→G2→G3.** **G0** ✅ 09-11. 🔴🔴 **G2 IS MEASURED OUT 09-12 — NOW A DECISION: (A) ESC RPM mode (physical, 4 sessions) or (B) close the loop in our own node (`px4_ros2` throttle setpoint types exist, no vehicle change). 🔑 DRIVE ON MEASURED DISPLACEMENT, NOT TIME.** **G3** = ⏭ **OPERATOR VENUE CALL** (gates T2).
