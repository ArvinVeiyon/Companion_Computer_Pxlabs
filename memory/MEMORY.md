...
⏭ **NEXT SESSION = `~/ros2_ws/docs/indoor_mapping_plan.md`** (architecture, all measured costs, step-by-step, open questions). **ORDER: camera reconfig 640×360 + `depth_registration` (one restart + half-dead check) → re-measure RTAB-Map → re-measure bag → manual mapping run → offline RTAB-Map on the laptop → AMCL/Nav2 on the Pi.** #20 yaw still gates autonomous driving. **O1 removed from the rover ladder**; lidar-owns-`/scan` remap PARKED.
