...
- `project_wfb_undervoltage_dead_nic.md` — **LIKELY FIXED 2026-07-25: XL4015 buck @5.25V, EXT5V ~5.00-5.09V, throttled 0x0, zero UV events. The "dead" NIC d993c0 is ALIVE ⇒ brownout WAS the cause; WFB_NICS back to BOTH.** The 155mV gap Pi-vs-DMM is a **PMIC ADC offset, NOT wiring loss** (266 samples, rail flat over 4A swing, ~6σ) — **DON'T raise the pot; keep as-read thresholds; DO NOT set usb_max_current_enable=1.** Watchdog `ext5v-logger.service` → run `ext5v-report 30` after the next ARMED run to close it. Note: `system_files_sync` SKIPS ENTIRELY when the FC reports armed ⇒ unreliable as a backup mid-session.
...
- ✅ **2026-07-26 CAMERA REMOUNT — AS-BUILT TF MEASURED + COMMITTED 07-27 (`ros2_ws f210102`). `depth_to_scan.launch.py` defaults are now the truth: cam_x 0.00 (rotation centre), cam_y 0.00, cam_z 0.305, cam_pitch 0.0406 (2.33° nose down), cam_roll 0.0100, range_max 5.0. Pitch/roll MEASURED from `/camera/accel/sample`, not assumed. Supersedes the 07-21 values (x −0.125 / z 0.420 / zero rpy) still quoted in todos #15 and older L4 text. NO LONGER BLOCKS L5 → [[project_l4_gemini_nav2_prereqs]]**
- `project_rover_autonav.md` — **ACTIVE. NEXT = yaw-gain tuning (#20), then L5 (slam_toolbox+Nav2).** L2 DONE (armed floor run PASSED) + reflex collision-stop pushed b38e413; all work pushed to origin/main through ae647a4. Arm workflow: arm in **Manual** via RC → software DO_SET_MODE→AutoNav (cannot arm AutoNav via RC). Bridge is start-by-hand, floor only. RC: ch2 throttle, ch4 steer, ch5 arm, ch6 mode, ch8 kill. **Hazards:** wheels-up + ekf-bridge + closed loop = limit cycle only disarm stops; never `pkill -f`/`pgrep -f` (self-matches the calling shell). Full resume steps + session checklist in the topic file + [[project_l2_floortest_wheel0_reversed]] + `ros2_ws/docs/yaw_tuning_session.md`.
...
- `project_ffmpeg_hung_alive_gap.md` — **2026-07-26 GS video cut-off: ffmpeg hung ALIVE (camera stopped feeding frames), watchdog only catches process DEATH ⇒ stream died silently. Includes fast non-root diagnosis via wfb API 8102 `video tx`. Red herrings ruled out: d993c0 0-pkt TX is NORMAL (mirror=False ⇒ single-best-NIC), Orbbec has no /dev/videoN by design.** — **SEQUEL OPEN 2026-07-27: GS feed now flaps on/off ~1×/min (59 ffmpeg starts in 67 min). Watchdog is HONEST (wfb incoming frozen 60 s during a reported stall); radio/power/thermal/Orbbec all ruled out. Cause = the fix's own SIGKILL recovery re-wedging the UVC device (`for 32s after 32s` = never got frame 1) + backoff reset bug at node:314. 4-item fix list agreed, to apply later 07-27.**
...
Cameras (roles set 2026-07-19): LG Smart Cam=FPV alias FPV, id usbcam-30c9009d-01.00.00-i00 (stable ids survive boot shuffles since v2.1). **NEVER record the camera's resolution/fps/bitrate as a fact in memory — it is operator-set from QGC and changes without notice (07-28: four docs asserted 1280x720 while the live conf ran 640x480). Read `/etc/vision_streaming.conf` or the running ffmpeg args when you need the current mode.** | Orbbec Gemini 336L=autonomy-only, color alias NAV-COLOR role_lock, id usbcam-2bc50807-CPC7B53000AB-i04 (**RECONNECTED — rover-camera+rover-scan active as of 2026-07-28, L5 not blocked**; up to 1280x800 MJPG; USB3 on BOX-B; ROS2 wrapper for phase3/4, never ffmpeg). **Orbbec /dev/videoN nodes APPEAR AND DISAPPEAR with `rover-camera.service`**: wrapper running ⇒ OrbbecSDK/libusb claims it, uvcvideo detaches, no Orbbec nodes; wrapper stopped ⇒ uvcvideo binds and creates 8 nodes, Orbbec taking video0. Both the old "video0=depth Z16/video2/4=IR" map AND the flat "has NO /dev/videoN at all" claim are wrong — the map is a function of service state. **/dev/video renumbering also comes from the Pi5's own `rpivid`+`pispbe-*` axi nodes (video19-37)** — LG cam was video8 at boot, video0, then video1 in one evening | old front/bottom cams removed | NEVER key cameras by /dev/videoN or /dev/v4l/by-id — only `usbcam-<vidpid>-<serial>-i<iface>` ids/aliases
