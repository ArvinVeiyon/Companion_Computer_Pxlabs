...
🔴 **REFLEX NOT SWITCHED YET — `/scan_3d` shows 16-24 beams <0.60 m.** Edge noise is real (median std 0.003 m inside ±30°, **0.058 m beyond 40°**; 40-43% of beams unstable past 30°) but **trimming the FOV barely helps (±46°→±34° drops near beams only 22→16)** ⇒ most are genuine room objects, so the reflex may already be behaving correctly. **Confirm in a genuinely clear space before switching.**
✅ **SETTLED 21:20 — the near returns ARE the rover's OWN TOP PLATE** (user identified it; docs agree). `docs/rover_autonav_requirements.md:75` **ground-to-top-plate = 0.235 m**, `cam_z 0.305 = 0.235 plate + 0.070 bracket`; MEASURED near band **z mean 0.231 m** — matches to **4 mm**. Looks like a constant-range ARC because the **0.308 m sensor near limit hides all but the outer ~4 cm sliver** of the deck. 🔴 **`range_min=0.40` IS CORRECT AND LOAD-BEARING — DO NOT lower it to 0.308**, that re-admits the plate as a permanent obstacle inside the bumper.
⚠️ **My 20:36 "overturned, it's room objects" claim was WRONG.** The two-pose test compared `/scan_3d` beams <0.60 m, but **`range_min=0.40` already CLIPS the plate (0.31-0.35 m)** — so it tested a population that could not contain it. **LESSON: check a filter doesn't already remove what you're testing for; verify at the CLOUD level.**
📐 **CAMERA MOUNT: LEAVE IT CENTRE** — (0,0,0.305)=0.235 plate+0.070 bracket, pitch 2.33°, 33.7 cm behind the bumper. Setback keeps a bumper-contact obstacle (slant 0.454 m) outside the 0.308 m near limit; the near blind zone is set by VFOV, so moving forward drags it along rather than shrinking it.
...
🔎 **`min_height=0.12` likely too conservative** — floor measured **−0.012 m**, ~6 cm below the 0.043-0.093 it was chosen against ⇒ **0.06-0.08 would roughly HALVE the 12 cm minimum visible obstacle. Measure 2-3 locations first.** ⚠️ but the top plate sits at **0.235 m**, inside the 0.12-0.45 band — it is `range_min`, not `min_height`, that excludes it.
