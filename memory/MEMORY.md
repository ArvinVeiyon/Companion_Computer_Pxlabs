...
- `project_external_wifi_uplink.md` — 2026-07-25: external USB RTL8821CU (wlx90de80d824d6) = PRIMARY uplink, netplan Nilan STATIC 192.168.1.240/24 metric50. Onboard wlan0 out of netplan (link DOWN). **CORRECTION 2026-07-26: `dtoverlay=disable-wifi` NEVER APPLIED — wlan0 still exists + brcmfmac still bound after the 07:57 boot. Cause = inline `#` comment on the same config.txt line (config.txt has no inline comments) → overlay silently dropped; `disable-bt` on the adjacent line DID work = clean control. FIX APPLIED 2026-07-26 (comment moved above the directive, backup `config.txt.bak.20260726-inlinecomment`) but **NOT REBOOTED — verify next boot** via `ip link show wlan0` + `lsmod | grep brcmfmac`. TODO #2 stays open until then**
- `project_wfb_undervoltage_dead_nic.md` — **LIKELY FIXED 2026-07-25 eve: companion power module → XL4015 buck @5.25V. EXT5V 4.65-4.84V → ~5.00-5.09V, throttled 0x50000→0x0, zero UV events. "Dead" NIC d993c0 is ALIVE (0 drops/200k pkts) and WFB_NICS is back to BOTH — so the brownout WAS the cause, the "faulty NIC" correction was wrong. Module now trimmed **5.20V under load (DMM-confirmed at output)**; Pi reads 5.045V — that 155mV gap is a **PMIC ADC OFFSET (~3%), NOT wiring loss** (earlier "100mΩ, re-crimp leads" was WRONG): 266 samples give slope +6.2mV/A, rail FLAT over a 4A core swing ⇒ resistance AND diode both excluded ~6σ; pot changes don't propagate. **DON'T raise the pot further** (5.20 real is near the 5.25 ceiling; chasing a displayed 5.20 ⇒ ~5.37V actual). **Keep using as-read thresholds** — PMIC trips on its own offset scale. WFB_NICS revert investigated: **NOT system_files_sync** (its rsync goes /etc→repo ONLY, never writes /etc) — it was a manual `cp -p` restore at 11:19:28 (ctime moved, mtime didn't), mitigation lived just 11:08:05→11:19:28. /etc edits do NOT revert on reboot. **BUT system_files_sync SKIPS ENTIRELY when the FC reports armed** — unreliable as a backup during work sessions. Watchdog `ext5v-logger.service` logs EXT5V+throttled+per-NIC TX drops @2s to /var/log/ext5v/; run `ext5v-report 30` AFTER THE NEXT ARMED RUN to close it. Still: DO NOT set usb_max_current_enable=1**
