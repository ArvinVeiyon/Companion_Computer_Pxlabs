...
📊 **RTAB-Map INSTALLED + MEASURED 08-02: IT DOES NOT FIT.** `rgbd_odometry` ALONE = **79.6% of a core**, load **2.25 → 9.44**, **`/scan` 13.4 → 7.0 Hz (halved)**, its own delay **~1 s** at ~3 Hz. **And that's without the map/loop-closure node OR the depth registration RGB-D needs.** ⇒ "blocked on config + CPU" CONFIRMED = **CPU**. ⏭ **Cheapest lever: colour is 1280×720 — drop to 640×360 (~4×), launch-time, needs the camera half-dead check after restart.**
