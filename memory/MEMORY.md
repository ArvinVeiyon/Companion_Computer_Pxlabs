...
⚠️ **Boot clock is WRONG until NTP steps it** — `uptime -s`, `who -b` and unit timestamps disagree across a boot; don't correlate journals across one. ⚠️ **Only 4 cores** → [VIDEO_FAULTS] (B). ⚠️ **No rate/CPU number is trustworthy without `ps -eo pid,pcpu --sort=-pcpu` first** — a "7.5 Hz cloud" was a runaway at 72.7%, and my own `claude` process once hit 130% and skewed a baseline.
...
**AIDE timer DISABLED** (~3.5h/day of a core). **tfmini DISABLED — ⚠️ MUST `systemctl enable --now tfmini` for the DRONE airframe.** **rover-ekf-bridge installed but DISABLED on purpose** (wheels-up limit-cycle hazard); ⚠️ it already uses `LocalPositionMeasurementInterface`, velocity-only — **the Q1 hook is half-built.**
...
✅ VL53L1X SUPERSEDED (dead code, retire with #17 + `collision_manual_mode` — everything is AutoNav-only). ⚠️ the reflex lives in `autonav_mode`'s executor ⇒ **not independent of the companion**; perception loss fails SAFE.
...
**Drone TX is flawless. When video breaks WFB's input queue is EMPTY — SUSPECT THE SOURCE.** Stats: TCP `127.0.0.1:8102` (GS 8103) JSON, **not** `wfb-cli`. **⚡ uplink loses 13.57% mavlink ⇒ NIC-A ant0 is 20 dB deaf (−48.5 vs −28.3).** `wifibroadcast@` restart's `rtw_mlmeext_disconnect` WARN+trace is **BENIGN**.
...
## [CURRENT STATE — 2026-08-08 19:20. NEVER ARMED]
✅ **#26 fixed + #28 reverted** (build needed `vision_streaming` STOPPED to dodge the CPU latch; restored, **operator CONFIRMED picture**). Camera restart verified NOT half-dead. **rtabmap localization LEFT RUNNING (~60% of a core) — KILL IT.** Operator has re-parked the rover in the ROOM CENTRE for 26b. Load 5–7.3/4 cores all session, **ffmpeg never lost the race**.
⚠️ **sudo NEEDS the password** — `printf '1987\n' | sudo -S …` (operator's own documented method). ⚠️ **`fps` is INERT in QGC**; resolution + bitrate work. **Replays need `ROS_DOMAIN_ID=42`; the LIVE stack is domain 0.** ⚠️ codex-work's push used the plaintext PAT ⇒ #14. **UNDECIDED: "del mirror one" — ask first.**
🖥 **Pi5 is NOT the bottleneck (measured): 5-41% idle, ~0% iowait, RAM 1.8/7.9 GB; everything heavy is ALREADY multi-threaded** (camera 41 threads, rtabmap 27, ffmpeg 19). ⛔ **NO GPU ENCODE EXISTS — `h264_v4l2m2m`="no valid device", `h264_vaapi`=no driver; card0=`v3d` (3D only), card1=`vc4-drm`. Headless changes NOTHING. Only real offload = a camera with ONBOARD H.264.**
...
23. ✅ watchdog LIVE (`a5fb348`). 25. ✅ rc_control retry storm FIXED, 🔴 **still keys `/dev/video0` — close out via Phase D/#8.**
26. ✅✅ **DEPTH-GLITCH ABORT FIXED** (`69d9c89`; patch `codex-work 16665f5`) — 1 h 23 m / 0 aborts, was ~13 min. ⚠️ **the Orbbec clone is GITIGNORED — a re-clone RESTORES the bug.** ⏭ untested: `align_mode:=HW` kills the class + saves 71% of a core. → indoor_mapping_slam §11
26b. 🔴🔴 **LOCALIZATION HAS NEVER RELOCALIZED — 0 accepted closures EVER. TEST = ≥1 ACCEPTED CLOSURE; `map->odom`/`/localization_pose` TICK REGARDLESS** ⇒ the 08-08 "works" claim is NOT evidence. ❌ ruled out: intrinsics · depth · CPU · `Vis/MinInliers`. Wall = 1466 rejects at 0 inliers (**a PLANE is DEGENERATE for PnP**); room centre = **ZERO candidates**. 📷 **GRAB THE FRAME — it beat 3 hypotheses in one step.** 🔴 **=Q1.** → indoor_mapping_slam §11-12
...
🔴 **Q1 "WHERE AM I?" IS THE WALL** — and 26b says localization has never actually solved it. Next intended step = `EKF2_EV_CTRL=9` fed by the **px4_ros2 Navigation Interface** (`rover_ekf_bridge` ALREADY uses `LocalPositionMeasurementInterface`, velocity-only; **experimental per PX4 docs**). PX4 RTL has NO obstacle avoidance here. Pending hw: DroneCAN GPS.
...
📊 **NO VIO EXISTS, don't build one** (08-08): `rgbd_odometry` is installed but launched nowhere and costs 79.6% of a core; the 336L's IMU is live but unfused/uncalibrated; `/odom` is WHEEL odom from ESC eRPM. **Wheel BEAT visual, 60 vs 4-5 closures** — RTAB-Map localizes by *place recognition*; odom only bridges between them. 🔑 the camera gyro IS an independent inertial yaw-rate source (PX4's `vehicle_angular_velocity` is NOT on DDS).
🔴 **Mapping is NOT hardware-blocked:** `slam_toolbox` needs 360°, but **RTAB-Map RGB-D is built for a narrow forward FOV.** **Swap the ALGORITHM, not the sensor.** 92° is the whole RGB-D class, not a 336L weakness.
